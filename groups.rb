...
# == FIND AND ADD ID OF SHIP TO GROUPS TABLE ==
 	...
 	def find_id(db)
		#convert/insert id - break into new method?
		find_id = 'SELECT id FROM ships WHERE name = ?'
		id = db.execute(find_id, [@ship_type])[0][0]
 	end

 	#Update groups table to include id of ship
 	def set_id(db)
		set_id = 'UPDATE groups SET ship_id = ? WHERE name = ?'
		db.execute(set_id, [find_id(db), @name])
 	end
...
# == CALCULATE AND ADD POINTS TO GROUPS TABLE ==
 	...
 	def find_points(db)
 		...
 		points = db.execute(find_points, [find_id(db)])[0][0]
 	end
